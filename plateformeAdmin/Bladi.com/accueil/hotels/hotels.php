            ...
            <div class="table-header" >
                ...
                <div id="search_add">
                    <form action="" method="get" style="display:inline">
                        <input placeholder="Hotel"   name="chercher" value="<?php if(isset($_GET['chercher'])){echo $_GET['chercher'];}?>">
                       <button class="add-new">Chercher</button>
                    </form>
                    <a href="./ajouter/ajouter.html"><button class="add-new"  > Ajouter hotel </button></a>
                </div>
            </div>
                        ...
                        <tbody>
                            <?php
                                ...
                                if(isset($_GET ['chercher']))
                                {
                                    ...
                                    $resultat = mysqli_query($connexion ,"SELECT * FROM hotels WHERE concat(nom,description,services,website,tel,atitude) Like '%$filteredValues%'"); 
                                    if(mysqli_num_rows($resultat)>0 ) 
                                    {
                                        ...
                                        <a href='delete.php?id=$ligne->id;' onclick=\"javascript:return confirm('Êtes-vous sûr de vouloir supprimer cet hôtel ? Cette action est irréversible et toutes les données associées à cet hôtel seront définitivement perdues. Veuillez prendre note que cette action ne peut pas être annulée. Êtes-vous certain de vouloir continuer ?');\"><button><i style='color:red' class='fa fa-trash' aria-hidden='true'  id='btn_' ></i></button>".'</td></tr>';
                                        }
                                    } 
                                    else {
                                        ?>
                                    ...
                                    $resultat = mysqli_query($connexion ,"SELECT * FROM hotels "); 
                                    if(mysqli_num_rows($resultat)>0 ) 
                                    {
                                        while($ligne = mysqli_fetch_object($resultat)) {
                                        echo '<tr><td>'. $ligne->id. ' </td><td>'.$ligne->nom.'</td><td>'.$ligne->tel.'</td><td>'.$ligne->website.'</td><td>'.$ligne->atitude.'</td><td>'.$ligne->longe.'</td><td>'."<a href='./modifier/modifier.php?id=$ligne->id'><button> <i class='fa fa-pencil' aria-hidden='true'></i></button></a>
